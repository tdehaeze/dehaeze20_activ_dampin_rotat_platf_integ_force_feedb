...
\hypersetup{colorlinks,bookmarksopen,bookmarksnumbered,citecolor=red,urlcolor=red}
\author{Dehaeze Thomas\affilnum{1,3} and Collette Christophe\affilnum{1,2}}
...
\end{abstract}
\keywords{Active damping, IFF, Rotating platform}
\date{}
...
The rotating stage is supposed to be ideal, meaning it induces a perfect rotation \(\theta(t) = \Omega t\) where \(\Omega\) is the rotational speed in rad/s.

The suspended platform consists of two orthogonal actuators represented by three elements in parallel: a spring with a stiffness \(k\) in N/m, a dashpot with a damping coefficient \(c\) in N/(m/s) and an ideal force source \(F_u, F_v\).
A payload with a mass \(m\) in kg, representing the sensitive equipment, is mounted on the (rotating) suspended platform.
...
\begin{equation}
  \omega_0 = \sqrt{\frac{k}{m}} \text{ in rad/s}, \quad \xi = \frac{c}{2 \sqrt{k m}}
\end{equation}

The elements of the transfer function matrix \(\bm{G}_d\) become equal to
\begin{subequations}
\label{eq:Gd_w0_xi_k}
  \begin{align}
...
For all further numerical analysis in this study, we consider \(\omega_0 = 1\,\text{rad}/s\), \(k = 1\,N/m\) and \(\xi = 0.025 = 2.5\,\%\).
...
\includegraphics[width=\linewidth]{campbell_diagram_real.pdf}
\caption{\label{fig:campbell_diagram_real} Real Part}
\end{subfigure}
\hfill
\begin{subfigure}[c]{0.45\linewidth}
\includegraphics[width=\linewidth]{campbell_diagram_imag.pdf}
...
\includegraphics[width=\linewidth]{plant_compare_rotating_speed_direct.pdf}
\caption{\label{fig:plant_compare_rotating_speed_direct} Direct Terms \(d_u/F_u\) and \(d_v/F_v\)}
\end{subfigure}
\hfill
\begin{subfigure}[c]{0.49\linewidth}
\includegraphics[width=\linewidth]{plant_compare_rotating_speed_coupling.pdf}
\caption{\label{fig:plant_compare_rotating_speed_coupling} Coupling Terms \(d_v/F_u\) and -\(d_u/F_v\)}
\end{subfigure}
\hfill
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{system_iff.pdf}
\caption{\label{fig:system_iff}System with added Force Sensor in series with the actuators}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{control_diagram_iff.pdf}
...
This can be seen in the Bode plot of the diagonal terms (Figure \ref{fig:plant_iff_compare_rotating_speed}) where the low frequency gain is no longer zero while the phase stays at \(180\,\text{deg}\).
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{plant_iff_compare_rotating_speed.pdf}
...
The direction of increasing gain is indicated by arrows.

\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{root_locus_pure_iff.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{loop_gain_modified_iff.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{root_locus_modified_iff.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{root_locus_wi_modified_iff.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{mod_iff_damping_wi.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{system_parallel_springs.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{cedrat_xy25xs.pdf}
...
\label{eq:Gk}
\begin{align}
\bm{G}_{k}(1,1) &= {\scriptstyle \frac{\left( \frac{s^2}{{\omega_0}^2} - \frac{\Omega^2}{{\omega_0}^2} + \alpha \right) \left( \frac{s^2}{{\omega_0}^2} + 2 \xi \frac{s}{\omega_0} + 1 - \frac{{\Omega}^2}{{\omega_0}^2} \right) + \left( 2 \frac{\Omega}{\omega_0} \frac{s}{\omega_0} \right)^2}{\left( \frac{s^2}{{\omega_0}^2} + 2 \xi \frac{s}{\omega_0} + 1 - \frac{{\Omega}^2}{{\omega_0}^2} \right)^2 + \left( 2 \frac{\Omega}{\omega_0} \frac{s}{\omega_0} \right)^2} } \\
\bm{G}_{k}(1,2) &= {\scriptstyle \frac{- \left( 2 \xi \frac{s}{\omega_0} + 1 - \alpha \right) \left( 2 \frac{\Omega}{\omega_0} \frac{s}{\omega_0} \right)}{\left( \frac{s^2}{{\omega_0}^2} + 2 \xi \frac{s}{\omega_0} + 1 - \frac{{\Omega}^2}{{\omega_0}^2} \right)^2 + \left( 2 \frac{\Omega}{\omega_0} \frac{s}{\omega_0} \right)^2} }
\end{align}
\end{subequations}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{plant_iff_kp.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{root_locus_iff_kp.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{root_locus_iff_kps.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{opt_damp_alpha.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{comp_root_locus.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{comp_transmissibility.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=1]{comp_compliance.pdf}
